...
class PatternLoader implements \Mustache_Loader {
	...
	protected function loadFile($name) {
		...
		$fileName = $this->baseDir."/".$this->patternUtil->getFileName($partialName,$this->extension);
		...
		if (count($styleModifier) > 0) {
			$fileData = $this->patternUtil->findReplaceParameters($fileData, $styleModifier);
		}
		
		// if the pattern name had parameters find & replace them
		if (count($parameters) > 0) {
			$fileData = $this->patternUtil->findReplaceParameters($fileData, $parameters);
		}
	...
	protected function getFileName($name) {
		...
		if (($posSlash === false) && ($posDash !== false)) {
			$fileName = $this->baseDir.$dirSep.$this->patternUtil->getPatternFileName($name);
		} else {
